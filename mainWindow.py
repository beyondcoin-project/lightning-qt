...
class MainWindow(QMainWindow):
    ...
    def createMenu(self):
        ...
        beyondcoin_menu = self.menu.addMenu("&Beyondcoin")
        beyondcoin_menu.addAction(self.get_address_segwit_action)
        beyondcoin_menu.addAction(self.get_address_p2sh_action)
    ...
    def getAddressP2sh(self):
        ...
        if address:
            QMessageBox.information(self, "Beyondcoin address", address["p2sh-segwit"])
    
    def getAddressBech(self):
        """Shows a message box containing a native segwit address (bech32)"""
        address = self.plugin.rpc.newaddr()
        if address:
            QMessageBox.information(self, "Beyondcoin address", address["bech32"])
