...
class Millibeyondcoin:
    """
    A subtype to represent thousandths of a micro-beyondcoin.

    Many JSON API fields are expressed in millibeyondcoins: these automatically get
    turned into Millibeyondcoin types.  Converts to and from int.
    """
    def __init__(self, v):
        """
        Takes either a string ending in 'mbynd', 'ubynd', 'bynd' or an integer.
        """
        if isinstance(v, str):
            if v.endswith("mbynd"):
                self.millibeyondcoins = int(v[0:-4])
            elif v.endswith("μbynd"):
                self.millibeyondcoins = Decimal(v[0:-3]) * 1000
            elif v.endswith("bynd"):
                self.millibeyondcoins = Decimal(v[0:-3]) * 1000 * 10**8
            else:
                raise TypeError("Millibeyondcoin must be string with mbynd/mbynd/bynd suffix or int")
            if self.millibeyondcoins != int(self.millibeyondcoins):
                raise ValueError("Millibeyondcoin must be a whole number")
            self.millibeyondcoins = int(self.millibeyondcoins)
        elif isinstance(v, Millibeyondcoin):
            self.millibeyondcoins = v.millibeyondcoins
        elif int(v) == v:
            self.millibeyondcoins = int(v)
        else:
            raise TypeError("Milli-Beyondcoin must be string with mbynd/mbynd/bynd suffix or int")

        if self.millibeyondcoins < 0:
            raise ValueError("Milli-Beyondcoin must be >= 0")

    def __repr__(self):
        """
        Appends the 'mbynd' as expected for this type.
        """
        return str(self.millibeyondcoins) + "mbynd"

    def to_microbeyondcoin(self):
        """
        Return a Decimal representing the number of Micro-Beyondcoins
        """
        return Decimal(self.millibeyondcoins) / 1000

    def to_bynd(self):
        """
        Return a Decimal representing the number of Beyondcoin
        """
        return Decimal(self.millibeyondcoins) / 1000 / 10**8

    def to_ubynd_str(self):
        """
        Return a string of form 1234mbynd or 1234.567mbynd.
        """
        if self.millibeyondcoins % 1000:
            return '{:.3f}mbynd'.format(self.to_microbeyondcoin())
        else:
            return '{:.0f}mbynd'.format(self.to_microbeyondcoin())

    def to_bynd_str(self):
        """
        Return a string of form 12.34567890bynd or 12.34567890123bynd.
        """
        if self.millibeyondcoins % 1000:
            return '{:.11f}bynd'.format(self.to_bynd())
        else:
            return '{:.8f}bynd'.format(self.to_bynd())
    ...
    def to_approx_str(self, digits: int = 3):
        ...
        while True:
            ...
            amount_rounded = round_to_n(self.millibeyondcoins, digits)
            # try different units and take shortest resulting normalized string
            amounts_str = [
                "%gbynd" % (amount_rounded / 1000 / 10**8),
                "%gmbynd" % (amount_rounded / 1000),
                "%gmbynd" % (amount_rounded),
            ]
    ...
    def __int__(self):
        return self.millibeyondcoins

    def __lt__(self, other):
        return self.millibeyondcoins < other.millibeyondcoins
    ...
    def __add__(self, other):
        return Millibeyondcoin(int(self) + int(other))

    def __sub__(self, other):
        return Millibeyondcoin(int(self) - int(other))

    def __mul__(self, other):
        return Millibeyondcoin(int(int(self) * other))

    def __truediv__(self, other):
        return Millibeyondcoin(int(int(self) / other))

    def __floordiv__(self, other):
        return Millibeyondcoin(int(self) // other)
...
class LightningRpc(UnixDomainSocketRpc):
    ...
    class LightningJSONDecoder(json.JSONDecoder):
        def __init__(self, *, object_hook=None, parse_float=None, parse_int=None, parse_constant=None, strict=True, object_pairs_hook=None):
            ...
            super().__init__(object_hook=self.millibeyondcoin_hook, parse_float=parse_float, parse_int=parse_int, parse_constant=parse_constant, strict=strict, object_pairs_hook=object_pairs_hook)

        @staticmethod
        def replace_amounts(obj):
            """
            Recursively replace _mbynd fields with appropriate values with Millibeyondcoin.
            """
            if isinstance(obj, dict):
                for k, v in obj.items():
                    if k.endswith('mbynd'):
                        if isinstance(v, str) and v.endswith('mbynd'):
                            obj[k] = Millibeyondcoin(v)
                        # Special case for array of mbynd values
                        elif isinstance(v, list) and all(isinstance(e, str) and e.endswith('mbynd') for e in v):
                            obj[k] = [Millibeyondcoin(e) for e in v]
                    else:
        ...
        def millibeyondcoin_hook(self, obj):
    ...
    def fundchannel(self, node_id, microbeyondcoin, feerate=None, announce=True, minconf=None):
        """
        Fund channel with {id} using {micro-beyondcoin} micro-beyondcoins
        ...
        payload = {
            "id": node_id,
            "micro-beyondcoin": micro-beyondcoin,
            "feerate": feerate,
            "announce": announce,
            "minconf": minconf,
        }
    ...
    def getroute(self, node_id, mbeyondcoin, riskfactor, cltv=9, fromid=None, fuzzpercent=None, exclude=[], maxhops=20):
        """
        Show route to {id} for {mbeyondcoin}, using {riskfactor} and optional
    ...
    def invoice(self, mbeyondcoin, label, description, expiry=None, fallbacks=None, preimage=None, exposeprivatechannels=None):
        """
        Create an invoice for {mbeyondcoin} with {label} and {description} with
    ...
    def pay(self, bolt11, mbeyondcoin=None, label=None, riskfactor=None, description=None):
        """
        Send payment specified by {bolt11} with {mbeyondcoin}
    ...
    def sendpay(self, route, payment_hash, description=None, mbeyondcoin=None):
        """
    ...
    def setchannelfee(self, id, base=None, ppm=None):
        """
        Set routing fees for a channel/peer {id} (or 'all'). {base} is a value in millibeyondcoin
        that is added as base fee to any routed payment. {ppm} is a value added proportionally
        per-millionths to any routed payment volume in micro-beyondcoin.
        """
        payload = {
            "id": id,
            "base": base,
    ...
    def withdraw(self, destination, microbeyondcoin, feerate=None, minconf=None):
        """
        Send to {destination} address {microbeyondcoin} (or "all")
        amount via Beyondcoin transaction. Only select outputs
        with {minconf} confirmations
        """
        payload = {
            "destination": destination,
            "microbeyondcoin": microbeyondcoin,
            "feerate": feerate,
            "minconf": minconf,
        }
        return self.call("withdraw", payload)

    def txprepare(self, destination, microbeyondcoin, feerate=None, minconf=None):
        """
        Prepare a beyondcoin transaction which sends to {destination} address
        {microbeyondcoin} (or "all") amount via Beyondcoin transaction. Only select outputs
        with {minconf} confirmations.
