...
from lightning import LightningRpc, Millibeyondcoin
...
class Plugin(object):
    ...
    def _coerce_arguments(func, ba):
        ...
        for key, val in ba.arguments.items():
            ...
            if annotation == Millibeyondcoin:
                args[key] = Millibeyondcoin(val)
            else:
                args[key] = val
        ba.arguments = args
        return ba
    ...
    def _multi_dispatch(self, msgs):
        ...
        for payload in msgs[:-1]:
            # Note that we use function annotations to do Millibeyondcoin conversions
